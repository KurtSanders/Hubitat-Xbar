...
# <xbar.desc>Provides a view and control of Hubitat and the sensors from the macOS menubar</xbar.desc>
# <xbar.image>https://raw.githubusercontent.com/KurtSanders/Hubitat-Xbar/main/Images/Hubitat-Xbar.png</xbar.image>
...
# XBARDarkMode = os.environ.get("XBARDarkMode")
...
if xbarOauthString is None:
    try:
        ...
        print ("Please set the required environment variables in the Xbar Pluggin", os.path.basename(scriptFile))
        exit(1)
...
if "error" in j:
    ...
    if j['error'] == 'invalid_token':
        print(":thumbsdown: Please verify that both the SmartApp URL and smartAppSecret in HE_Python_Logic.cfg are correct.")
...
hsmStates = ["armedAway", "armedHome", "armedNight", "disarmed", "armedHome", "disarmed", "disarmed", "armedAway", "disarmed"]
        ...
        for x in range(len(mainDisplay)):
            ...
            if isinstance(mainDisplay[x]['value'], int) or isinstance(mainDisplay[x]['value'], float):
                ...
                print("{} | {} {} dropdown=false".format(formattedMainDisplay, 'size=14', mainMenuColor))
                 ...
                 print(" | image={} dropdown=false".format(utilities.getImageString(mainDisplay[x]['emoji'])))
            else:
...
#                mainDisplay[x]['value'] = hsmCurrentStateDict.get(mainDisplay[x]['value'][3:], mainDisplay[x]['value'][3:])
                ...
                for thermoMode in thermoModeList:
                    if thermoMode != thermostat['thermostatMode']:
                        ...
                        print("----• {}".format(TitleCase(thermoMode)), buildFontOptions(3), \
                              ...
                              " param3=" + smartAppSecret, thermo_param4, ' terminal=false refresh=false')
                    ...
                    for c in range(currentCoolingSetPoint - 5, currentCoolingSetPoint):
                        ...
                        print("----• ", str(c) + degree_symbol, buildFontOptions(3), \
                              ...
                              "bash=", callbackScript, " param1=request param2=", str(
                                coolSetpointURL + str(c)), " param3=", smartAppSecret, thermo_param4, " terminal=false refresh=false")
                    ...
                    for c in range(currentHeatingSetPoint + 5, currentHeatingSetPoint, -1):
                        ...
                        print("----• ", str(
                            c) + degree_symbol, buildFontOptions(3), "color=", numberToColorGrad(id, "red"), \
                              "bash=" + callbackScript, " param1=request param2=" + str(
                                heatingSetpointURL + str(c)), " param3=" + smartAppSecret, thermo_param4, " terminal=false refresh=false")
        ...
        if mode['name'] not in currentmode['name']:
            ...
            print("--• " + mode[
                'name'], buildFontOptions(3), colorText, ' bash=' + callbackScript, ' param1=request param2=' + currentModeURL, \
                  ' param3=' + smartAppSecret, mode_param4, ' terminal=false refresh=false')
        colorSwitch = not colorSwitch
...
if hsmDisplayBool and hsmState is not None:
    if modes is None:
        hortSeparatorBar()
    colorText = ''
    print("Hubitat™ Safety Monitor is {}{}".format(hsmState,buildFontOptions()))
    # Verify the HSM is configured:
    if hsmState != "unconfigured":
        print("--Select to Change" + buildFontOptions())
        for hsmNewState in hsmStates:
            colorText = 'color=#333333' if colorSwitch else 'color=#666666'
            if hsmNewState == hsmState:
                currenthsmDisplay = " (Current)"
                currenthsmURL = ""
            else:
                currenthsmURL = hsmURL + hsmState
                currenthsmDisplay = ""
                hsm_param4 = 'param4=\"{} {}{}\"'.format('Setting HSM to', hsmState.title(), currenthsmDisplay)
                currenthsmURL = 'bash=' + callbackScript + ' param1=request param2=' + currenthsmURL + \
                                  ' param3=' + smartAppSecret, ' param4=' + hsm_param4, ' terminal=false refresh=false'
#            print("--• {}{}".format(hsmState.title(), currenthsmStateDisplay), buildFontOptions(3), \
#                  colorText, currenthsmmURL)
            colorSwitch = not colorSwitch
            ...
            if sensor['isRGB'] is True and colorChoices > 0:
                ...
                for colorChoice in colorChoices:
                    ...
                    print(subMenuText + "{:>3}. {} ".format(count, getHueLevel(colorChoice)), buildFontOptions(4), \
                          'bash=' + callbackScript, ' param1=request param2=' + currentColorURL, ' param3=' + smartAppSecret, \
...
print("--• Launch macOS Default Browser to View STXbarAPP-V2 GitHub Software Resp " + buildFontOptions() + openParamBuilder("open https://github.com/kurtsanders/STXbarApp-V2") + ' terminal=false')
